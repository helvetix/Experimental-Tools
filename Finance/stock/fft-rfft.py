...
def sineWaveGenerator2(fftList, index):
    ...
    for t in fftList[1:]:
        ...
        value = np.sin(point * np.pi / 180.0) * t[1]
        result = result + value            
...
print "actual\n", actual[:10]
# print "actual1\n", actual1[:10]
yf = np.fft.rfft(actual)
yf = np.abs(yf)

print "FFT length=%d  bias=%f  max=%f  fundamental=%d" % (len(yf),  getFFTBias(yf), np.max(yf[1:]), np.argmax(yf[1:])+1)
print "fundamental frequency %d" % (getFFTFrequency(yf, np.argmax(yf[1:])+1) )
